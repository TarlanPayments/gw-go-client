// Package tarlanpaymentsgateway provide ability to make requests to Tarlan Payments Gateway API v3.
package tarlanpaymentsgateway
...
import (
	...
	"github.com/TarlanPayments/gw-go-client/operations"
	"github.com/TarlanPayments/gw-go-client/structures"
)
...
const (
	dAPIBaseURI = "https://api.sandbox.tarlanpayments.kz"
	dAPIVersion = "3.0"
)

type (
	// confAPI, endpoint config to Tarlan Payments system
	confAPI struct {
...
func NewGatewayClient(AccountID int, SecretKey string) (*GatewayClient, error) {
	if AccountID == 0 {
		return nil, errors.New("Account ID can not be 0, please use the given ID from Tarlan Payments")
	}
...
// NewRequest method, send HTTP request to Tarlan Payments API
...
// determineURL the full URL address to send request to Tarlan Payments API
